# 🧑‍💻 Anis Ajengui  
**Tech Lead & Full-Stack Developer** | **PHP & Symfony Expert** | **NestJS, Express, React**

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=ajenguianis&label=Profile%20views&color=0e75b6&style=flat" alt="Profile views" />
</p>

---

## 🌟 À propos de moi

> 💡 *"Le code doit être clair, concis et fonctionnel. L’explication devrait être superflue."*

Je suis un **Tech Lead Full-Stack** avec plus de **8 ans d’expérience** dans le développement d’applications web à haute performance. Spécialisé en **PHP (Symfony, Laravel)**, je maîtrise également les écosystèmes **Node.js**, **React**, et **Angular**.

Mon expertise va bien au-delà du codage : je suis un **architecte logiciel**, un **leader technique** et un **manager de projet** capable de piloter des équipes, de concevoir des systèmes robustes, et de garantir la qualité, la scalabilité et la sécurité des solutions.

J’ai une solide expérience dans l'**architecture logicielle**, la **gestion de projet agile**, la **CI/CD**, la **DevSecOps**, et l'**optimisation des performances**. Je développe aussi des outils pour automatiser les workflows DevOps et améliorer les processus métier, notamment dans le domaine de l’e-commerce (PrestaShop, Magento) et des applications métier complexes.

---

## 📈 Statistiques GitHub

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=ajenguianis&show_icons=true&theme=tokyonight)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=ajenguianis&layout=compact&theme=tokyonight)

---

## 🛠️ Compétences

### ✅ Architecture Logicielle
- Conception d’architectures modulaires et évolutives
- Mises en œuvre de microservices / monolithe
- Bonnes pratiques DDD, Clean Architecture, SOLID
- Gestion de la sécurité (OWASP, Authentification, RBAC)

### 🧩 Tech Lead & Management
- Pilotage d’équipes de développement
- Gestion de projet Agile (Scrum, Kanban)
- Priorisation des tâches et estimation des sprints
- Mentorat de développeurs juniors/moyens

### 🔧 Back-end
- **PHP**: Symfony 5/6, Laravel, Doctrine
- **Node.js / NestJS / Express**
- **REST APIs / GraphQL**
- **Clean Architecture / DDD**

### 🖥️ Front-end
- **Angular 13+**
- **React + TypeScript**
- **Tailwind CSS / Bootstrap**
- **Responsive Design**

### 🧰 DevOps & Infrastructure
- **Docker / Docker Compose**
- **GitLab CI/CD / GitHub Actions**
- **Ansible / Terraform**
- **Bash Scripting / CI/CD Pipelines**
- **Cloud (AWS, Azure, GCP)**

### 🗄️ Bases de données
- **MySQL / PostgreSQL**
- **MongoDB / Redis**
- **Elasticsearch**

### 📦 Outils & Frameworks
- **API Platform**
- **PrestaShop / Magento**
- **Jira / Confluence / Notion**
- **Git / GitFlow / Trunk-Based Development**

---

## 🧩 Projets Clés

### 📦 [ux-analyzer](https://github.com/ajenguianis/ux-analyzer)
**Description**: A Node.js tool for UI/UX analysis and modernization, extracting styles, accessibility, and performance metrics from web pages (e.g., Brevo’s Next.js/Tailwind sites). Detects frameworks (Tailwind, Bootstrap), analyzes colors/typography, audits WCAG 2.1, and generates AI-ready prompts. Features screenshots, performance metrics (FCP, LCP), and deep analysis mode.

### 📦 [symfony-git-review](https://github.com/ajenguianis/symfony-git-review)
**Description**: A reusable shell script for generating automated code review prompts for Symfony projects. It analyzes Git diffs, scans project context, and creates detailed review prompts focusing on Symfony best practices, clean architecture (CQRS, DDD, SOLID), and code quality. Supports AI integration (GitHub Copilot, Claude, GPT) and stores outputs in `.vscode/` for git-ignored local storage.

---

### 📦 [prompt-engineering-library](https://github.com/ajenguianis/prompt-engineering-library)
**Description**: Une bibliothèque collaborative de prompts optimisés pour utiliser efficacement les outils d’IA dans le développement logiciel et les opérations IT.

---

### 📦 [ftp-sftp-connector](https://github.com/ajenguianis/ftp-sftp-connector)
**Description**: Un **librairie PHP** qui simplifie le travail avec les protocoles **FTP et SFTP**, offrant une interface facile à utiliser pour le transfert de fichiers, la liste des répertoires distants, etc.

---

### 📦 [easy-file-parser](https://github.com/ajenguianis/easy-file-parser)
**Description**: Une librairie PHP conçue pour **parser divers formats de fichiers** tels que **TXT, CSV et XLSX**. Elle fournit des composants réutilisables pour une extraction et une manipulation efficaces des données.

---

### 📦 [ps-module-setting](https://github.com/ajenguianis/ps-module-setting)
**Description**: Une librairie PHP destinée à **gérer et récupérer les paramètres de configuration** dans les projets **PrestaShop**. Elle permet de gérer les configurations via une base de données ou un fichier `.env`.

---

### 📦 [musicSphereAngularApp](https://github.com/ajenguianis/musicSphereAngularApp)
**Description**: Une application Angular 18 pour la gestion de musiciens, groupes, concerts et salles. Elle s'appuie sur l'API backend développée avec **Symfony**.

---

### 📦 [musicSphereApi](https://github.com/ajenguianis/musicSphereApi)
**Description**: Une API RESTful Symfony 6 pour gérer des bandes musicales, leurs membres, des concerts, des salles de concert, et des importations de fichiers.

---

## 🌐 Où me trouver

- 📘 [CV en ligne](https://ajenguianis.github.io/cv-Anis-Ajengui/)
- 📧 [Email](mailto:anis.ajengui@gmail.com)
- 📘 [LinkedIn](https://www.linkedin.com/in/anis-ajengui-93560789/)
- 📦 [GitHub](https://github.com/ajenguianis)

---

## 💬 Demandez-moi sur :

- **Architecture logicielle** (DDD, Clean Architecture, Microservices)
- **Tech Lead** (gestion d’équipe, priorisation, roadmap)
- **Symfony** (bundles, performances, architecture)
- **Laravel** (routes, Eloquent, artisan)
- **Node.js / NestJS / Express**
- **Angular / React**
- **CI/CD, DevOps, Docker**
- **API Platform** (REST, GraphQL, sécurité)
- **PrestaShop / Magento**

---

🧠 *“Le code doit être clair, concis et fonctionnel. L’explication devrait être superflue.”*
